...
void Mesh::begin(uint8_t _channel, uint16_t _node_id)
{
  ...
  if(DEBUG) printf("MESH: Info: Initializing Node: id: %u, address: 0%o.\n\r",
...
bool Mesh::send(Payload& payload, uint16_t to_id)
{
  ...
  if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Sending payload to %d: %s...", 
...
void Mesh::update()
{
  ...
  if ( network.available() ) {
    ...
    if(DEBUG) printf("MESH: Info: %u, 0%o: Got message %c from 0%o.\n\r",
...
void Mesh::read(Payload& payload)
{
  ...
  if(header.type == 'M') {
    ...
    if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Received payload: %s.\n\r",
...
bool Mesh::send_P()
{
  ...
  if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Sending ping...", 
...
void Mesh::handle_P(RF24NetworkHeader& header)
{
  ...
  if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Received ping from %d.\n\r",
  ...
  if(DEBUG) printf("MESH: Info: %u, 0%o: Address map is updated: %s.\n\r", 
...
bool Mesh::send_A(const uint16_t new_address)
{
  ...
  if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Sending new address '0%o'.\n\r", 
...
void Mesh::handle_A(RF24NetworkHeader& header)
{
  ...
  if(DEBUG) printf("MESH: Info: %u, %s, %d byte: Received address '0%o'.\n\r",
...
void Mesh::set_address(uint16_t address)
{
  ...
  if(DEBUG) printf("MESH: Info: Reinitializing Node: id: %u, new address: 0%o.\n\r",
              node_id, node_address);
  // apply new address
  network.begin(channel, node_address);
  // send ping to base
